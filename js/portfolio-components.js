(function () {
  ...
  function renderEcpc(assetRoot) {
    const slides = data.activity.ecpc.map((slide, index) => {
      ...
      const front = future
        ? `<div class="ecpc-deck-card ecpc-future-card"><div class="ecpc-future-aura" aria-hidden="true"></div><img class="ecpc-future-logo" src="${asset(assetRoot, slide.image)}" alt="Egyptian Collegiate Programming Contest" loading="lazy" decoding="async" /><span class="ecpc-photo-number">${String(index + 1).padStart(2, '0')} / ${String(data.activity.ecpc.length).padStart(2, '0')}</span><span class="ecpc-future-label">Next contest</span></div>`
    ...
    return `
      <section class="ecpc-showcase activity-feature-card premium-card reveal" aria-labelledby="activity-ecpc-heading">
        ...
        <div class="ecpc-body"><div id="ecpcDeck" class="ecpc-deck" role="region" aria-roledescription="carousel" aria-label="Hassan's ECPC experiences" tabindex="0"><div class="ecpc-deck-stage"><div class="ecpc-slider-track" aria-live="polite">${slides}</div><div class="ecpc-deck-controls"><button class="ecpc-deck-arrow" id="ecpcPrev" type="button" aria-label="Previous ECPC story"><i class="fas fa-arrow-left" aria-hidden="true"></i></button><button class="ecpc-deck-arrow" id="ecpcNext" type="button" aria-label="Next ECPC story"><i class="fas fa-arrow-right" aria-hidden="true"></i></button></div></div><div class="ecpc-deck-indicators" aria-label="Choose an ECPC chapter">${indicators}</div></div></div>
      </section>`;
  }
